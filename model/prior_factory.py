...
def uniform(batch_size, n_dim, n_labels=4, minv=-1, maxv=1, label_indices=None):
    if label_indices is not None:
        if n_dim != 2 or n_labels != 4:
            raise Exception("n_dim must be 2 and n_labels must be 4.")
...
def gaussian(batch_size, n_dim, mean=0, var=1, n_labels=4, use_label_info=False):
    if use_label_info:
        if n_dim != 2 or n_labels != 4:
            raise Exception("n_dim must be 2 and n_labels must be 4.")
...
def gaussian_mixture(batch_size, n_dim=2, n_labels=4, x_var=0.5, y_var=0.5, label_indices=None):
    if n_dim != 2:
...
def swiss_roll(batch_size, n_dim=2, n_labels=4, label_indices=None):
    if n_dim != 2:
        raise Exception("n_dim must be 2.")
